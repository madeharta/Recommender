...
with mlflow.start_run(run_name="TSLA_model_selection"):
    ...
    plt.plot(test_dates, actual_next_close_test.values, label="Actual")
    plt.plot(test_dates, naive_pred_close, label="Naive")
    plt.plot(test_dates, pred_next_close, label=best_name)
    plt.xlabel("Date")
    plt.ylabel("Price ($)")
